        ...
        <form onSubmit={submit} className="grid gap-4 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm md:grid-cols-2">
          {fields.map(([name, label, required]) => (
            <label className={name === "keyFeatures" || name === "tone" ? "grid gap-1.5 text-xs font-bold text-neutral-700 md:col-span-2" : "grid gap-1.5 text-xs font-bold text-neutral-700"} key={name}>
              <span>{label}</span>
              {name === "keyFeatures" ? (
                <textarea className="min-h-24 rounded-xl border border-neutral-200 px-4 py-3 text-xs font-semibold outline-none focus:border-neutral-300 focus:ring-2 focus:ring-neutral-100" name={name} required={required} />
              ) : name === "platform" ? (
                <select className="rounded-xl border border-neutral-200 px-4 py-3 text-xs font-semibold outline-none focus:border-neutral-300 focus:ring-2 focus:ring-neutral-100" name={name} defaultValue="meesho">
                  <option value="meesho">Meesho</option>
                  <option value="flipkart">Flipkart beta</option>
                  <option value="amazon">Amazon beta</option>
                </select>
              ) : name === "tone" ? (
                <select className="rounded-xl border border-neutral-200 px-4 py-3 text-xs font-semibold outline-none focus:border-neutral-300 focus:ring-2 focus:ring-neutral-100" name={name} defaultValue="marketplace optimized">
                ...
                <input className="rounded-xl border border-neutral-200 px-4 py-3 text-xs font-semibold outline-none focus:border-neutral-300 focus:ring-2 focus:ring-neutral-100" name={name} required={required} />
              )}
            </label>
          ))}
...
function EditableBlock({ label, value, tall = false }: { label: string; value?: string; tall?: boolean }) {
  return (
    <label className="grid gap-1.5 text-xs font-bold text-neutral-700">
      <span>{label}</span>
      <textarea className={`rounded-xl border border-neutral-200 p-3 text-xs font-semibold leading-6 outline-none focus:border-neutral-300 focus:ring-2 focus:ring-neutral-100 ${tall ? "min-h-28" : "min-h-16"}`} defaultValue={value || ""} />
    </label>
  );
}
