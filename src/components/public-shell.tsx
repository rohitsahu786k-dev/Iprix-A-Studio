      ...
      <div className="container flex min-h-[80px] items-center justify-between gap-6 relative">
        ...
        <nav className="hidden items-center gap-1.5 rounded-full border border-zinc-850 bg-zinc-900/40 p-1 text-sm font-semibold text-zinc-400 lg:flex">
          {publicLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="relative rounded-full px-4.5 py-2 transition-all duration-200 hover:bg-zinc-800 hover:text-zinc-100 hover:shadow-sm"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          {session ? (
            <div className="flex items-center gap-3">
              <Link 
                className="text-sm font-semibold text-zinc-350 hover:text-zinc-100 transition-colors duration-200 inline-flex items-center gap-1 group" 
                href="/dashboard"
              >
                Dashboard
              ...
              <div className="flex flex-col gap-0.5">
                {publicLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="rounded-xl px-3.5 py-2 text-xs font-bold text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100 transition-all"
                  >
                    {link.label}
                  </Link>
                ))}
...
export function PublicFooter() {
  return (
    <footer className="border-t border-zinc-800 bg-white pt-20 pb-12 text-zinc-600">
      <div className="container">
        {/* Top Section */}
        <div className="grid gap-12 pb-16 md:grid-cols-[1.5fr_1fr_1fr_1.2fr] border-b border-zinc-800">
          ...
          <div className="space-y-6">
            ...
            <div className="inline-flex items-center gap-2 rounded-2xl border border-zinc-800 bg-zinc-900/50 px-3.5 py-2.5 text-xs font-bold text-zinc-600">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-zinc-400">Quick Links</h3>
            <ul className="space-y-2.5 text-sm text-zinc-600">
              {publicLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="hover:text-indigo-650 transition-colors flex items-center gap-1 group">
                    <ChevronRight className="h-3.5 w-3.5 opacity-0 group-hover:opacity-100 transition-opacity -ml-3.5 group-hover:ml-0 duration-200 text-indigo-650" />
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Product Links */}
          <div className="space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-zinc-400">Product</h3>
            <ul className="space-y-2.5 text-sm text-zinc-600">
              {[
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <h3 className="text-xs font-extrabold uppercase tracking-widest text-zinc-400">Get In Touch</h3>
              <div className="grid gap-3 text-sm text-zinc-600">
                <a className="inline-flex items-center gap-2 hover:text-indigo-650 transition-colors" href={`mailto:${brand.supportEmail}`}>
                  <Mail className="h-4 w-4 text-zinc-600" />
                  {brand.supportEmail}
                </a>
                <a className="inline-flex items-center gap-2 hover:text-indigo-650 transition-colors" href={brand.whatsappUrl}>
                  <MessageCircle className="h-4 w-4 text-zinc-600" />
                  {brand.whatsappDisplay}
                </a>
                <span className="leading-relaxed text-xs text-zinc-650">{brand.address}</span>
              </div>
            </div>
        ...
        {/* Bottom Section */}
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-zinc-650">
