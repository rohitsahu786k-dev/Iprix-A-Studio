  ...
  return (
    <div className="space-y-7">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-medium tracking-tight text-zinc-100 sm:text-4xl">Overview</h1>
      </div>
          ...
          <article className="rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-md p-5 shadow-sm" key={key}>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 break-words text-2xl font-medium text-zinc-100">{formatValue(key, stats[key] ?? 0)}</p>
              </div>
...
function RecentPanel({ title, items }: { title: string; items: Array<Record<string, unknown>> }) {
  return (
    <article className="rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-md p-5 shadow-sm">
      <h2 className="text-sm font-medium uppercase tracking-wider text-zinc-100">{title}</h2>
        ...
        {items.length ? (
          items.map((item, index) => (
            <div className="rounded-xl border border-zinc-900 bg-zinc-950/50 p-3 text-xs text-zinc-300" key={String(item._id || index)}>
              <p className="break-words font-medium text-zinc-100">{String(item.name || item.email || item.feature || item.status || "Activity")}</p>
