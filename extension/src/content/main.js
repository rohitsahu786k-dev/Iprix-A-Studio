...
import { detectCurrentCategory, scrapeBreadcrumbCategory } from "./category.js";

(function () {
  // Polyfills and safe guards for chrome APIs to prevent context-invalidated crashes
  ...
  async function fillForm(templateData, { resolvedCategory } = {}) {
    ...
    if (!resolvedCategory) {
      const templateCategory = (templateData.category || "")
        ...
        .toLowerCase();
      if (templateCategory) {
        // Detect the page category from every available source. Any one hitting
        // is enough — Meesho renders these at different times, so we try them all.
        const detected = (
          detectCurrentCategory() ||
          scrapeBreadcrumbCategory() ||
          sessionStorage.getItem("listify_tab_category") ||
          sessionStorage.getItem("listify_tab_category_full") ||
          ""
        )
        ...
        // Only BLOCK on a confirmed mismatch. If we cannot detect the page
        // category at all (timing / DOM variation), proceed with the fill —
        // blocking on a detection failure was the "Category not detected" bug.
        // A loose containment check tolerates leaf-vs-full-path differences.
        if (detected) {
          const matches =
            detected === templateCategory ||
            detected.includes(templateCategory) ||
            templateCategory.includes(detected);
          if (!matches) {
            showToast(
